...
describe('EventDetailScreen', () => {
  ...
  it('ignores a second moderation tap while the first request is in flight', async () => {
    let resolveDecision!: (value: { ok: true }) => void;
    decideRequestMock.mockReturnValue(new Promise((resolve) => {
      resolveDecision = resolve;
    }));
    fetchEventMock.mockResolvedValue({ ok: true, event: communityEvent({ isHost: true }) });
    const event = communityEvent({
      isHost: true,
      teamRoster: BASE_ROSTER,
      joinRequests: [{
    ...
    const approve = renderer!.root.findByProps({ accessibilityLabel: 'Schválit žádost @karel' });

    act(() => {
      approve.props.onPress();
      approve.props.onPress();
    });
    expect(decideRequestMock).toHaveBeenCalledTimes(1);
  ...
  it('shows the real event status', () => {
    let renderer: ReturnType<typeof TestRenderer.create>;
    act(() => {
      renderer = TestRenderer.create(<EventDetailScreen event={communityEvent({ status: 'live' })} />);
    });

    expect(flatTexts(renderer!)).toContain('Právě teď');
  });
