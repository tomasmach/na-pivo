...
export interface QuizAnswer {
  ...
  at: number;
  /** Server order resolves equal client clocks. */
  cursor?: number;
}
...
export interface QuizState {
  ...
  /** Scores through the previous question, safe to show before this reveal. */
  previousStandings: QuizStanding[];
  finished: boolean;
}
...
function teamAnswer(
  ...
  for (const answer of answers) {
    ...
    const order =
      answer.at - best.at ||
      (answer.cursor ?? 0) - (best.cursor ?? 0) ||
      answer.entrantId.localeCompare(best.entrantId) ||
      answer.option - best.option;
    if (order < 0) best = answer;
  }
  return best;
}
...
export function quizState({
  ...
  const scoreBoard = (included: readonly QuizQuestion[]): QuizStanding[] => teams
    .map((team) => ({
      teamId: team.id,
      teamName: team.name,
      score: included.reduce((score, item) => {
        const answer = teamAnswer(answers, entrants, team.id, item.id);
        return answer && answer.option === item.answer ? score + 1 : score;
      }, 0),
    }))
    .sort((a, b) => b.score - a.score);
  const standings = scoreBoard(questions);
  const previousStandings = scoreBoard(questions.slice(0, Math.max(0, index)));
