...
describe('quiz rules', () => {
  ...
  it('breaks equal client timestamps deterministically', () => {
    const left = { ...answer('p1', 'a', 0, 10), cursor: 9 };
    const right = { ...answer('p2', 'a', 1, 10), cursor: 8 };

    expect(quizState({ entrants: TEAMS, answers: [left, right], index: 0, questions: QUESTIONS }))
      .toEqual(quizState({ entrants: TEAMS, answers: [right, left], index: 0, questions: QUESTIONS }));
  });
  ...
  it('does not expose the current answer through the scoreboard before reveal', () => {
    ...
    expect(state.standings.find((row) => row.teamId === 'Ty')?.score).toBe(1);
    expect(state.previousStandings.find((row) => row.teamId === 'Ty')?.score).toBe(0);
  });
});
