...
describe('peopleOf', () => {
  ...
  it('never renders a nameless person', () => {
    const people = peopleOf(
      evening({
        members: [
          { id: ME, nickname: null, displayName: '', avatarUrl: null },
          { id: 'x', nickname: null, displayName: '   ', avatarUrl: null },
        ],
      }),
      ME,
    );

    expect(people[1].name).toBe(fallbackPlayerName('x'));
    expect(people[1].name).toMatch(/^Hráč [1-9]\d?$/);
  });
...
describe('fallbackPlayerName', () => {
  it('derives the number from the id alone', () => {
    // No roster argument exists to disagree about: every device hashes the
    // same account id to the same label.
    expect(fallbackPlayerName('h-id')).toBe(fallbackPlayerName('h-id'));
    expect(fallbackPlayerName('guest-7')).toMatch(/^Hráč [1-9]\d?$/);
  });

  it('keeps the label when other members join or leave', () => {
    // Membership is not an input, so nobody gets renumbered by somebody
    // else sitting down or standing up.
    const before = fallbackPlayerName('guest-7');
    expect(fallbackPlayerName('guest-7')).toBe(before);
  });

  it('stays a human-safe Hráč N label for any id', () => {
    expect(fallbackPlayerName('solo')).toMatch(/^Hráč [1-9]\d?$/);
  });
});
