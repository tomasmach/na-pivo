 ...
 *
 * Two countrywide boards (Objevitelé · Mapéři) over three windows
...
export default function LeaderboardsScreen({ embedded = false }: LeaderboardsScreenProps) {
  ...
  const [category, setCategory] = useState<LeaderboardCategory>('pubs');
  ...
  const chooseCategory = useCallback(
    (index: number) => {
      const next = CATEGORIES[index] ?? 'pubs';
