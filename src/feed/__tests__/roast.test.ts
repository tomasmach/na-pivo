...
describe('buildRoast — what it is allowed to be rude about', () => {
  ...
  it('does not celebrate a high drinking pace', () => {
    // Eight beers in an hour is fast; the roast must not read it as an
    // achievement, so it stays silent — the retired pace brag is gone for good.
...
describe('buildRoast — retired copy', () => {
  it('keeps the retired pace brag out of the roast source', () => {
    const source = readFileSync(join(__dirname, '..', 'roast.ts'), 'utf8');
    expect(source).not.toContain('rychlejší než obvykle');
  });
});
