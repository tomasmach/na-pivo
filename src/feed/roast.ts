...
export function buildRoast(input: RoastInput): Roast | null {
  ...
  if (input.pubs >= 3 && input.photos === 0) {
    return {
      line: `${input.pubs} hospody a ani jedna fotka`,
  ...
  if (input.games > 0 && input.gamesWon === 0) {
    return {
      line: `${input.games === 1 ? 'Jedna hra' : `${input.games} hry`}, ani jedna výhra`,
  ...
  if (ratio >= 2) {
    return {
      line: `${input.beers} piv za ${formatDuration(input.duration)} a pořád jen na jedno`,
      basis: `Tvoje tempo je ${Math.round(ratio)}× rychlejší než obvykle`,
    };
  }
  ...
  if (ratio <= 0.5 && input.duration >= 180) {
    return {
      line: `${formatDuration(input.duration)} a ${input.beers} piv`,
