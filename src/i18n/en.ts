...
export const en: Strings = {
  ...
  friends: {
    ...
    publicToursHeader: 'Public tours',
  ...
  tours: {
    ...
    empty: "Pick 2 to 8 pubs, send the crew a link and head out. On the way I'll show how far it is on foot and what's on tap.",
    ...
    publicState: 'Public', publicHiddenState: 'Hidden after reports', fromPublic: 'From public tours', publicTour: 'Public tour',
    seenByAll: 'Everyone will see', notSeenByOthers: 'Others will not see',
    seenTitle: (title: string) => `The name “${title}”`,
    seenStops: (pubs: string, distance: string) => `${pubs} in this order, about ${distance} on foot`,
    seenChallenges: (n: number) => englishPlural(n, { one: 'A challenge at 1 pub', other: `Challenges at ${n} pubs` }),
    seenProfile: 'Your nickname and profile', hiddenMeetup: 'Meetup date and time', hiddenPrivate: 'Your beers and location',
    publishNameNote: 'If the name includes someone’s name, better change it. Everyone will see the name.',
    publishUndoNote: 'You can remove it from public at any time. Anyone who saved it meanwhile keeps their copy.',
    rulesPrefix: 'By publishing you agree to the ', rulesLink: 'public tour rules', rulesSuffix: '.',
    publishAction: 'Publish', publishedNotice: 'The tour is public.', rulesUrl: 'https://tomasmach.github.io/na-pivo/en/terms.html#verejne-tour',
    signIn: 'Sign in', pickNickname: 'Pick a nickname', openPrivacy: 'Open privacy',
    report: 'Report', reportTitle: 'Report this tour?',
    reportMessage: 'You will stop seeing it. If more people report it, it is hidden for everyone until I take a look.',
    reported: 'Reported. You will not see it again.', reportFailed: 'The report did not go through. Try again.',
    backToTours: 'Back to Tour de pub', publicNewer: 'Public · you have newer changes',
    privateHidesPublic: 'A hidden profile also hides your public tours.', authorA11y: (nickname: string) => `Profile of the author ${nickname}`,
    publicMeta: (city: string, pubs: string, distance: string) => [city, `${pubs}, about ${distance} on foot`].filter(Boolean).join(' · '),
    crewAlone: 'No crew yet', crewNone: 'No crew', crewInvite: 'Invite your crew', crewInviteMore: 'Invite more', crewInviteHint: 'Shows a QR code for your crew',
    ...
    crewPing: 'Ping the crew', crewPingAway: 'Mates who aren’t here', crewPingHint: 'Tells your mates from Parta where you are sitting',
    crewPingNote: 'I’ll tell them in the app that you’re on a tour and where you’re sitting. Meet up at the pub.',
    crewPingNoteSolo: 'I’ll tell your mates from Parta in the app that you’re on a tour and where you’re sitting. Meet up at the pub.',
    crewPingFrom: (pub: string) => `Ping the crew from ${pub}`, crewPingTo: (pub: string) => `Ping the crew on the way to ${pub}`,
    crewPingSent: (pub: string) => `Your mates see ${pub}`, crewPingQueued: 'I’ll ping them once you get a signal.',
    ...
    crewOptOut: 'Don’t count me', crewOptIn: 'Count me',
    crewLeave: 'Leave the tour', crewLeaveTitle: 'Leave the tour?', crewLeaveMessage: 'Your walk ends and your crew will see you are no longer with them.',
    crewInvitedBy: (name: string) => `${name} is taking you on a tour`,
    crewAlreadyGoing: (name: string, others: number) => `${name} and ${others} more are going`,
    crewJoin: 'I’m in', crewSignInJoin: 'Sign in and join',
    crewJoinPrivacy: 'Your crew will only see that you walk with them. Pubs and beers stay on your phone.',
    crewActiveRun: 'You already have a tour under way.', crewActiveRunMessage: 'End it and join the crew?', crewEndAndJoin: 'End and join',
    crewHistory: (n: number, counted: boolean) => [n > 1 ? `${n} in the crew` : 'No crew', counted ? 'counted' : null].filter(Boolean).join(' · '),
    peopleCount: (n: number) => n > 0 ? englishPlural(n, { one: '1 person did this tour', other: `${n} people did this tour` }) : 'Nobody has done it yet',
    publicWithPeople: (n: number) => `Public · ${englishPlural(n, { one: '1 person did it', other: `${n} people did it` })}`,
    discoverTitle: 'Public tours', discoverOpen: 'Search public tours', discoverPlaceholder: 'City, pub or tour name', discoverClear: 'Clear search',
    discoverNearMe: 'Near me', discoverStops: 'Number of pubs', discoverStopsA11y: (chosen: string) => `Number of pubs, ${chosen}`, discoverStopsTitle: 'How many pubs?', discoverStopsAny: 'Doesn’t matter',
    discoverStopsRange: { '2-3': '2 to 3 pubs', '4-5': '4 to 5 pubs', '6-8': '6 to 8 pubs' },
    discoverChallenges: 'With challenges', discoverNoLocation: 'I don’t have your location, type a city or a pub.',
    discoverNothingNear: 'Nobody has published anything within 25 km of you yet. These are the nearest:', discoverNothingNearFiltered: 'Nothing within 25 km of you matches these filters. These are the nearest:',
    discoverPlanOwn: 'Plan your own tour', discoverNothing: 'Nothing here yet. Plan the first one and publish it.',
    discoverNothingFiltered: 'Nothing matches these filters.', discoverNothingFor: (q: string) => `Nothing for “${q}”. Try a city or another pub.`, discoverClearFilters: 'Clear filters',
    discoverOffline: 'Without a connection I can’t load new tours. Your own tours stay on your phone.', discoverFailed: 'That didn’t load.',
    discoverPeople: (n: number) => englishPlural(n, { one: 'walked by 1 person', other: `walked by ${n} people` }),
    discoverWithChallenges: 'with challenges', discoverDistanceA11y: (distance: string) => `${distance} from you`,
    challenge: 'Challenge', challengeLabel: 'Challenge:', challengeA11y: (text: string) => `Challenge: ${text}`,
    addChallenge: 'Add a challenge', editChallenge: 'Edit challenge', saveChallenge: 'Save challenge',
    removeChallenge: 'Remove challenge', writeChallenge: 'Write a challenge', challengeHint: 'Something everyone at the table can do.',
    challengePlaceholder: 'For example: ask the bartender how long they’ve been pouring here',
    challengeCount: (length: number, max: number) => `${length}/${max}`,
    challengeCountA11y: (left: number) => englishPlural(left, { one: '1 character left', other: `${left} characters left` }),
    ...
    published: 'Published version', publishChanges: 'Update link', unpublished: 'Your phone has a newer plan.',
    ...
    rail: 'Tour', railA11y: 'Open Tour de pub', otherTours: 'Other tours', pastRuns: 'Past',
    continueRun: 'Back to the tour', createNew: 'Plan a new one', upNext: 'Up next',
    ...
    errors: {
      ...
      publicSignIn: 'Only signed-in users can publish.', publicNickname: 'Pick a nickname first. It appears on your public tours.',
      publicPrivate: 'Public tours need a public profile.', publicRules: 'Accept the public tour rules first.',
      publicText: 'I won’t publish this. Public tours can’t be about who drinks more or faster, and can’t include links or phone numbers.',
      publicTitle: (title: string) => `I won’t publish the name “${title}”. Public tours can’t be about who drinks more or faster, and can’t include links or phone numbers.`,
      publicThrottled: 'Too many tries at once. Try again in a moment.',
      publicChallenge: (name: string) => `${name}: I won’t publish this challenge. Public challenges can’t be about who drinks more or faster, and can’t include links or phone numbers.`,
      publicUnknownPub: (name: string) => `${name}: I don’t know this pub for a public tour. Swap it for a pub from search.`,
      publicHiddenPub: (name: string) => `${name}: the community hid this pub. Swap it for another one.`,
      publicHidden: 'This tour is hidden after reports. I’ll take a look.', publicLimit: 'You can have at most 10 public tours. Remove one first.',
      publicNetwork: 'No connection, so I can’t publish. The tour is still on your phone.',
    },
  },
