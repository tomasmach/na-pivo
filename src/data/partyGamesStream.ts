...
type StreamOutcome = 'delivered' | 'empty' | 'limited';
  ...
  return new Promise((resolve) => {
    ...
    xhr.onreadystatechange = () => {
      ...
      if (xhr.readyState === 4) {
        if (xhr.status === 429) {
          try {
            const body = JSON.parse(xhr.responseText) as { code?: unknown };
            if (
              body &&
              typeof body === 'object' &&
              body.code === 'stream_limit_reached'
            ) {
              finish('limited');
              return;
            }
          } catch {
            // Not JSON — keep the existing behavior.
          }
        }
        finish(spoke ? 'delivered' : 'empty');
      }
...
export function subscribeToPartyGames(
  ...
  const emitGames = (games: PartyGame[]) => {
    const changed = games.filter((game) => {
      ...
      const isChanged = gameSnapshots.get(game.id) !== snapshot;
      knownGames.add(game.id);
      gameSnapshots.set(game.id, snapshot);
      return isChanged;
    });
    if (changed.length > 0) handlers.onGames?.(changed);
  };
      ...
      const outcome = await openStream(
        ...
        (frame) => {
          ...
          try {
            ...
            // A `start` event can also mean that the first confirmed lobby
            // bound a roster to a cover already on the table. Do not fold its
            // cursor yet: reconnect through catch-up so the updated game row
            // reaches `onGames` before gameplay starts on this phone.
            if (event.kind === 'start') {
              unknownGame = true;
              connection.abort();
              return;
