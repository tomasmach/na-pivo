...
function SearchContent() {
  ...
  const renderResult = (result: PubSearchResult, metadata?: string) => (
    <Pressable key={result.id} accessibilityRole="button"
      accessibilityLabel={[result.name, result.location || [result.address, result.city].filter(Boolean).join(', '), metadata].filter(Boolean).join(', ')}
      ...
      {metadata ? <Text style={styles.metadata} maxFontSizeMultiplier={FontScaleCap.body}>{metadata}</Text> : null}
    ...
    <View style={styles.root}>
      <View style={[styles.screen, { paddingTop: insets.top + Spacing.sm }, mapPub && styles.hidden]}>
        ...
        <KeyboardAwareScrollView contentContainerStyle={[styles.body, { paddingBottom: insets.bottom + Spacing.xl }]}
          ...
          {!term && frequent.length > 0 ? <>
            <Text style={styles.heading} accessibilityRole="header" maxFontSizeMultiplier={FontScaleCap.heading}>{t.pubSearch.frequent}</Text>
            {frequent.map(({ pub, visitCount }) => renderResult({ ...pub, pub }, t.pubSearch.visitCount(visitCount!)))}
          </> : null}
...
const styles = StyleSheet.create({
  ...
  cancelText: { fontFamily: Fonts.ui.medium, fontSize: 14, color: Colors.amber },
  body: { paddingHorizontal: Spacing.lg },
  heading: { fontFamily: Fonts.display.bold, fontSize: 22, lineHeight: 28, color: Colors.foam, marginTop: Spacing.md, marginBottom: Spacing.xs },
  ...
  metadata: { fontFamily: Fonts.ui.regular, fontSize: 13, color: Colors.mutedText, maxWidth: '30%', textAlign: 'right' },
