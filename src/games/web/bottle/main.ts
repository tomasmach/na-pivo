...
/** The bottle, as a lathe profile: body, shoulder, neck, lip. */
function bottleMesh(colour: string): THREE.Mesh {
  const profile: THREE.Vector2[] = [
    new THREE.Vector2(0, 0),
    new THREE.Vector2(0.42, 0),
    new THREE.Vector2(0.42, 1.5),
    new THREE.Vector2(0.36, 1.85),
    new THREE.Vector2(0.16, 2.25),
    new THREE.Vector2(0.16, 2.95),
    new THREE.Vector2(0.2, 3.05),
    new THREE.Vector2(0, 3.05),
  ];
  const geometry = new THREE.LatheGeometry(profile, 40);
  // Lying down and pointing along +Z, so "where it points" is one rotation.
...
class BottleTable {
  ...
  constructor(
    ...
    this.bottle = bottleMesh(theme.accent);
    this.bottle.castShadow = true;
    this.bottle.position.y = 0.42;
