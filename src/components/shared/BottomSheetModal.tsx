...
export function bottomSheetKeyboardOffset(keyboardHeight: number): number {
...
export function BottomSheetModal({
  ...
  /**
   * Keeps the React subtree alive only. Native Modal views (focus/scroll) are
   * not persistent; callers must lift draft state or restore native position.
   */
  keepMounted?: boolean;
  /** Lift and constrain the card host above a modal-window keyboard. */
  keyboardLift?: boolean;
  /** Stable owner for globally coordinated launch surfaces. */
  presentationId?: string;
}) {
  const { height } = useWindowDimensions();
  const reduceMotion = useReducedMotion();
  const presentation = useModalPresentation(visible, presentationId);
  ...
  const [mounted, setMounted] = React.useState(presented);
  ...
  if (presented && !mounted) setMounted(true);
  ...
  React.useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [presented, height, mounted, reduceMotion, scrim, translateY]);
  ...
  return (
    ...
    >
      ...
      {keyboardLift ? (
        <KeyboardLiftedCard animatedStyle={cardStyle}>{children}</KeyboardLiftedCard>
      ) : (
  ...
  return (
    <Animated.View
      style={[
        styles.card,
        animatedStyle,
        { paddingBottom: bottomSheetKeyboardOffset(keyboardHeight) },
      ]}
