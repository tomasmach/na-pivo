...
- **Seed data migration** mirrors `0031_seed_beer_brands` exactly: `0037_seed_amenity_kinds` does idempotent `update_or_create` over the v1 set, with a `reverse_code` that deletes the seeded keys. The original v1 seed was **16 active + 2 reserved** (the reserved rows are seeded with `active=False`). Follow-up data migrations `0040_deactivate_unused_amenity_kinds` and `0041_amenity_identity_tombstones` then deactivate five of those active rows (`seating_kids_corner`, `payment_cash_only`, `atmosphere_dogs_welcome`, `practical_food`, `atmosphere_smoking`) via the deactivation contract, dropping the active set to **11 active + 7 inactive** (2 reserved + 5 deactivated). The rows are retained (not deleted) so existing votes/aggregates survive:
...
| ~~`payment_cash_only`~~ | ~~Jen hotovost~~ | ~~Hotovost~~ | ~~payment~~ | ~~`BanknoteIcon`~~ | ~~true~~ | **false** (deactivated by 0040 — exact inverse of `payment_card`) | ~~20~~ |
...
| ~~`seating_kids_corner`~~ | ~~Dětský koutek~~ | ~~Děti~~ | ~~seating~~ | ~~`BabyIcon`~~ | ~~true~~ | **false** (deactivated by 0040 — unnecessary) | ~~50~~ |
| `game_darts` | Šipky | Šipky | games | `TargetIcon` | true | true | 60 |
| `game_billiards` | Kulečník | Kulečník | games | `CircleDotIcon` (lucide `CircleDot` — a cue ball) | true | true | 70 |
| `game_foosball` | Stolní fotbal | Fotbálek | games | `SoccerBallIcon` (custom hand-rolled `react-native-svg` glyph) | true | true | 80 |
...
| ~~`atmosphere_dogs_welcome`~~ | ~~Psi vítáni~~ | ~~Psi~~ | ~~atmosphere~~ | ~~`DogIcon`~~ | ~~true~~ | **false** (deactivated by 0040 — dropped by product) | ~~120~~ |
| ~~`atmosphere_smoking`~~ | ~~Kuřárna / kouření povoleno~~ | ~~Kouření~~ | ~~atmosphere~~ | ~~`CigaretteIcon`~~ | ~~true~~ | **false** (deactivated by `0041`, low-signal / sensitive venue attribute) | ~~130~~ |
...
| ~~`practical_food`~~ | ~~Kuchyně / dá se najíst~~ | ~~Kuchyně~~ | ~~practical~~ | ~~`UtensilsIcon`~~ | ~~true~~ | **false** (deactivated by 0040 — assumed for a pub, low signal) | ~~160~~ |
| `practical_outdoor_tap` | Venkovní výčep | Výčep | practical | *(none — assign before activation; NOT `BeerIcon`)* | false | **false** (reserved, seeded `active=False`) | 170 |
| `practical_tank_beer` | Tankové pivo | Tank | practical | `BeerIcon` | true | **false** (reserved, seeded `active=False`) | 180 |

The **active set = exactly the 11 rows with `active=True`**; the seven `active=False` rows are present in the seed/catalogue but excluded from `GET /kinds`, the sheet, and from BOTH the numerator and denominator of completeness — two RESERVED (`practical_outdoor_tap`, `practical_tank_beer`, seeded `active=False`) plus five DEACTIVATED by migrations `0040_deactivate_unused_amenity_kinds` and `0041_amenity_identity_tombstones` (`seating_kids_corner`, `payment_cash_only`, `atmosphere_dogs_welcome`, `practical_food`, `atmosphere_smoking`). So **`total_kinds` (active) = 11** and per-pub completeness can reach 100%.

> **`game_foosball` icon — the one custom glyph.** Lucide ships NO soccer/football icon and the project has no other icon pack, so `SoccerBallIcon` is a deliberate hand-rolled `react-native-svg` glyph in the mobile `src/components/shared/IconGlyph.tsx` (outer circle + central pentagon + radial seams) — the single intentional exception to the "lucide owns all icon paths" rule. The backend just stores the export-name string `SoccerBallIcon`; the custom path lives mobile-side.

Notes locked from the taxonomy dimension: keys are **group-prefixed, lowercase snake_case** with exactly five group prefixes — `payment_`, `seating_`, `game_`, `atmosphere_`, `practical_`. Note the deliberate asymmetry: the prefix is `game_` (singular) but the `Group` value is `games` (plural). Smoking is currently inactive; if reactivated it stays ONE tri-state amenity (`atmosphere_smoking`: yes = lze kouřit/kuřárna, no = nekuřácká) because modeling `kuřárna` and `nekuřácká` as separate keys recreates the boolean-conflation trap. `0040_deactivate_unused_amenity_kinds` deactivates four initially-active keys: `payment_cash_only` (exact inverse of `payment_card` — card=yes ⇒ cash-only=no, so asking both was redundant and could contradict), `seating_kids_corner` (unnecessary), `atmosphere_dogs_welcome` (dropped by product), and `practical_food` (assumed for a pub, low signal). `0041_amenity_identity_tombstones` deactivates `atmosphere_smoking`. `practical_outdoor_tap` is a **reserved key, seeded with `active=False`** (assign a glyph and activate later in a taxonomy bump; it must NOT reuse `BeerIcon`). `IconGlyph.tsx` cross-repo dependency: ~13 client glyphs to add — all one-line lucide wraps EXCEPT `SoccerBallIcon` (`game_foosball`), which is a custom hand-rolled `react-native-svg` glyph because lucide has no soccer icon; flagged for mobile, not a backend concern. The deactivated amenities never render, so their glyphs (`BanknoteIcon`, `BabyIcon`, `DogIcon`, `UtensilsIcon`, `CigaretteIcon`) are not needed.
  ...
  "pubs": [
    {
      "cache_key": "u2fkbnhz",
      "mapper_count": 5,
      "completeness": {"mapped_count": 6, "total_kinds": 11, "pct": 0.55},
      "amenities": [
...
- `completeness` is a **nested object** `{mapped_count, total_kinds, pct}` (the canonical shape — NOT a flat float with sibling fields): `mapped_count` = distinct ACTIVE amenity_keys with `status != "unknown"`; `total_kinds` = count of active `AmenityKind` = **11** (server-authoritative denominator, the client never hard-codes it); `pct` = `mapped_count / total_kinds`, float 0.0–1.0 clamped to [0,1], the per-pub meter ("zmapováno z N %").
...
4. **`payment_card` vs `payment_cash_only` contradictions.** ~~Modeled as two independent keys (sheet suggests-not-forces the opposite). A meaningful share of users will tap both as "bere obojí," producing flagged yes/yes conflicts the aggregate must tolerate.~~ **Resolved (migration `0040`):** `payment_cash_only` is deactivated as the exact inverse of `payment_card` (card=yes ⇒ cash-only=no). Only `payment_card` stays active, so there are no two-key contradictions to tolerate.
