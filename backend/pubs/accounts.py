...
def _validate_account_merge_operation(
    ...
    *,
    ...
    target_fingerprint = account_merge_fingerprint(target_public_id)
    source_fingerprint = (
        account_merge_fingerprint(source_public_id) if source_public_id is not None else None
    )
    if operation.target_account_fingerprint != target_fingerprint:
        raise AccountError(
    ...
    if (
        ...
        and operation.source_account_fingerprint != source_fingerprint
    ):
        raise AccountError(
...
def cancel_deletion(account: Account) -> bool:
    ...
    with transaction.atomic():
        ...
        AccountDeletionOperation.objects.filter(
            account_fingerprint=account_deletion_fingerprint(locked.public_id)
        ).delete()
