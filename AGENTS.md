...
- Po dokončení úkolu (commit, push, PR) ukliď build balast worktree, ve kterém jsi pracoval. Jeden iOS build v něm nechá několik GB. Hlavní checkout a worktree s cizí běžící session vynech. Když necháváš appku k proklikání, úklid počká. Jinak nejdřív zastav vlastní session (`npm run dev:stop` u detached běhu, jinak vlastní PID) a zkontroluj ruční nativní změny v `ios/` a `android/`. Pak smaž `ios/`, `android/`, `.expo/dev-client-ios-*`, `.expo/xcodebuild.log`, `node_modules` a `backend/.venv`. V `~/Library/Developer/Xcode/DerivedData` smaž jen složku `Napivo-*`, jejíž `WorkspacePath` v `info.plist` je přesně `<kořen worktree>/ios/Napivo.xcworkspace`. Chybějící `ios/` nový build nevynutí, fingerprint na něm nezávisí. Před další prací spusť v kořeni `npm ci` a v `backend/` `uv sync --locked`. Sdílené cache (`*.noindex` v DerivedData, CocoaPods, npm), simulátory a appku v nich nech, používají je ostatní worktrees.
